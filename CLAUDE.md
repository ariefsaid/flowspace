...
- App lives at the **repo root** (Next.js App Router). `app/` = routes/layouts/route-handlers; `components/` =
  ...
  formatters; the app DDL lives in ordered `supabase/migrations/` (`0000_app_schema.sql` first) with the Drizzle query
  layer in `lib/db/schema.ts` (+ `drizzle/` reference artifacts; drizzle-kit is **not** the DDL authority); `e2e/` = Playwright acceptance tests (the BDD layer).
