...
import type { Booking, BookingStatus, PaymentStatus } from "@/lib/mock/types";
...
function paymentBadgeTone(payment: PaymentStatus) {
  switch (payment) {
    case "PAID_ONLINE":
...
function ActiveBookingCard({ booking }: ActiveBookingCardProps) {
  const member = members.find((m) => m.activeSession?.table === booking.facility);
  ...
  return (
    <div className="rounded-xl border-2 border-green-400 bg-white shadow-md overflow-hidden">
      ...
      <div className="p-4 space-y-3 bg-green-50/40">
        ...
        {/* CTA */}
        <button className="w-full flex items-center justify-center gap-2 rounded-xl bg-teal-600 hover:bg-teal-700 text-white font-medium text-sm py-3 transition-colors">
...
export default function AdminBookingsPage() {
  ...
  const confirmedCount = 0; // no CONFIRMED status in mock
