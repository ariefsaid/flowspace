...
export type BookingHistoryView = {
  ...
  /** ISO start timestamp. */
  start: string;
  /** ISO end timestamp, or null for an in-progress walk-in. */
  end: string | null;
...
/** Display kind derived from the ledger TransactionType enum. */
export type TransactionKindView = "cafe" | "print" | "package" | "booking";

export type TransactionHistoryView = {
  id: string;
  kind: TransactionKindView;
  description: string;
  amount: number;
  /** ISO timestamp. */
  datetime: string;
...
function KindIcon({ kind }: { kind: TransactionKindView }) {
  const base = "h-4 w-4";
  switch (kind) {
    case "cafe":
...
export function HistoryClient({ bookings, transactions }: HistoryClientProps) {
