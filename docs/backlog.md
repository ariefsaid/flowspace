...
## CURRENT STATE (2026-08-29 — source-parity wave complete) — read this first
- **Source-parity wave DONE (2026-08-29, main CI green).** The client's original app source was obtained + audited (`docs/audit-verdict.md` + `docs/gap-analysis-original.md` = the behavior oracle); FlowSpace is now at functional/behavioral parity while keeping the architectural lead. Merged: **I-041** tiers (PR #7), **I-046** RLS write-lockdown (PR #8), **I-043** print parity (PR #9) + **PDF hotfix** (PR #10), **I-040** booking overhaul (PR #11, flagship), **I-044** cafe nuances (PR #12). See **Phase 4** below for per-issue detail.
- **Stack:** Supabase (Postgres+Auth+Realtime+Storage+RLS) + Drizzle, server-authoritative (ADR-0013/0014/0015). Prisma/Neon/NextAuth removed. Migrations **`0000`–`0019`**.
- **Tests on `main` (CI green):** ≈ unit 440 / int 297 / e2e 9 (curated). Every business read/write is `org_id`-scoped and server-authoritative; RLS revokes client write DML (I-046); money paths (booking credits/checkout, print pricing, cafe variants/discount) hardened by cross-family review.
...
- **Open for owner:** **I1** print non-PDF billing decision (on the I-045 line); **I-042** admin CRUD + **I-045** integrations unstarted. All review follow-ups filed in Phase-4-follow-ups + tech-debt below.
...
- [~] **I-042** Admin settings pages — **IN FLIGHT** (`feat/i-042-admin-settings`): wire the 7 dead settings cards — facilities CRUD · cafe-menu CRUD · print-pricing editor · site/analytics/email/unifi config pages (org_settings store, migration 0020; UniFi/email stay simulated seams pending I-045). Bulk pending-approve already shipped in I-040.
- [ ] **I-047** Admin console completion (from the 2026-08-29 final gap sweep — all file-evidenced): users Add/Edit/soft-archive + password reset (stubs today, `UsersClient.tsx:118,225`) · manual "Tambah Booking" (stub, `BookingsClient.tsx:489`) · wire the existing `cancelBooking` repo fn to a Batalkan action (dead code today, `lib/db/bookings.ts:1213`) · admin "Aktifkan Sekarang" manual-activation fallback (cron-only today — a paid CONFIRMED member is locked out if the sweep misfires) · print-reports search/status/date filters + restore the pending-jobs stat tile (ours shows "Pengguna Aktif" instead) · POS lookup shows facility+end-time, add clear-cart.
- [ ] **I-048** `/admin/reports` page (L): revenue trend + revenue-by-type + booking-stats charts, daily/weekly/monthly selector (ORIG `admin/reports/page.tsx`, 284 lines; we have no route at all).
- [ ] **I-049** Member-surface parity polish (final gap sweep): cafe "Pesanan Terakhir" recent orders w/ live status (render path exists, server passes null) · guest checkout shows the order code (`placeOrder` return discarded) · barista sound+toast on new orders (spec'd: recon 0001:127 — toggle is inert) · ORIG's 8-clause booking policy box (ours is a one-liner) · dashboard: gate QR/WiFi/quick-access on an active session + empty-state CTA card, per-tier badge colors, per-booking simulated WiFi voucher w/ "Get Voucher" flow (constant today) · extension 1h/2h choices + "blocked by next booking" pre-check message · topup: confirm dialog + method display + simulated processing + 5% simulated decline path + `?tab=` deep-link · signup persists phone (needs a column) · seed: sample transactions + one member per tier.
