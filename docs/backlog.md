...
- **Merged to main 2026-06-22 (CI green, `4edeab1`):** debt-cleanup (dead lib/mock + stale drizzle/ removed, spec-0002 traceability) · cafe 5% active-session discount (AC-115, 3-reviewer SHIP) · `/admin/print-reports` real over print_jobs (I-026, spec 0005, 3-reviewer + design re-review SHIP-CLEAN) · `(admin)` layout `requireRole("ADMIN")` guard (defense-in-depth, security SHIP). Migrations unchanged (0000–0007). Unit 202 / int 28+.
...
- **Local env:** 3 Supabase CLI stacks run side-by-side (flowspace `34321/34322`, pmo-portal `54xxx`, gordi-mos `44xxx`); the repo `.env` must point at flowspace (`DATABASE_URL=…:34322`). Re-seed after any `test:int` (it truncates the shared dev DB).
