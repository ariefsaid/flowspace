...
- [ ] **Design-system a11y debt (not I-026 drift):** the `DESIGN.md` hero gradient (`from-teal-500 to-teal-600` + white/teal-100 text) fails WCAG AA contrast (~2.2–2.5:1), and `text-orange-600` accent on white is borderline (3.56:1). Both are app-wide tokens — fix at the design-system level (darken gradient / adjust subtext), not per-page.
