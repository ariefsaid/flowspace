...
- **In review (branches, unpushed, awaiting owner merge — 2026-06-22):** `chore/debt-cleanup` (delete dead lib/mock + stale drizzle/ + spec-0002 traceability) · `feat/cafe-discount-activate` (5% active-session discount, AC-115, 3-reviewer SHIP) · `feat/admin-print-reports` (I-026 print-reports for real over print_jobs, spec 0005, 3-reviewer + design re-review SHIP-CLEAN).
...
- [x] **`/admin/print-reports` built for real** (2026-06-22, branch `feat/admin-print-reports`, I-026, spec 0005) — org-scoped report over `print_jobs`; 3-reviewer + design re-review SHIP-CLEAN. Follow-ups: **(a)** add an `(admin)` layout `requireRole("ADMIN")` guard (defense-in-depth across ALL admin pages — they currently rely on middleware only; security M1); **(b)** compute the summary aggregates via SQL independent of the 500-row cap, or paginate, before volume grows (security M2); **(c)** CSV/PDF export + date-range filter deferred.
