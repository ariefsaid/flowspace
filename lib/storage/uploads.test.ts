 ...
 *
 * AC-0239-storage: upload path is org-scoped (orgId prefix + deterministic structure).
 * AC-0240-storage: invalid MIME types are rejected before any storage call.
 *
...
describe("buildPrintStoragePath", () => {
  it("AC-0239-storage: path starts with orgId (org-scoped)", () => {
    const path = buildPrintStoragePath("org-abc", "job-123", "report.pdf");
    expect(path).toMatch(/^org-abc\//);
  });

  it("AC-0239-storage: path includes print/ segment + jobId directory", () => {
    const path = buildPrintStoragePath("org-abc", "job-123", "report.pdf");
    expect(path).toBe("org-abc/print/job-123/report.pdf");
  });

  it("AC-0239-storage: different orgs produce different path prefixes", () => {
  ...
  it("AC-0239-storage: sanitises file name (strips leading slashes and path traversal)", () => {
  ...
  it("AC-0239-storage: truncates very long file names to 200 chars", () => {
...
describe("validatePrintFile", () => {
  it("AC-0240-storage: accepts PDF MIME type", () => {
    expect(() =>
      validatePrintFile("application/pdf", 1024 * 1024)
    ).not.toThrow();
  });

  it("AC-0240-storage: accepts Word MIME types", () => {
    expect(() =>
