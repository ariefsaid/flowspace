...
const pages = await parallel([
  ...
  pageTask('admin /admin dashboard', `${ROOT}/app/(admin)/admin/page.tsx (+ components/admin/* you own)`, null, 'OBS-020..023',
    'No screenshot — build from spec + DESIGN.md. "Admin Dashboard" + subtitle; row of 4 KPI StatTiles then row of 3 revenue StatTiles (formatRupiah), then a "Recent Transactions" card listing recentTransactions (user, description, datetime, amount, status Badge). Use the EXACT English labels shown in OBS-021/022/023 of the spec (read them verbatim from the spec file, including punctuation).'),
